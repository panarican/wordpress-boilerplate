...
define( 'CUSTOM_METABOXES_DIR', get_template_directory_uri() . '/inc/class/metaboxes' );
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'select':
		case 'chosen':
			echo '<select  id="' . esc_attr( $id ) . '"' , $type == 'chosen' ? ' class="chosen"' : '' , isset( $multiple ) && $multiple == true ? ' name="' . esc_attr( $name ) . '[]" multiple="multiple"' : ' name="' . esc_attr( $name ) . '"' , '>
					<option value=""></option>'; // Select One
			

		if (isset( $multiple )) {
			foreach ( $options as $option ) {
				$isSelected =  (in_array( $option['value'], $meta ) ) ? 'selected="selected"' : '';
				echo '<option ' . $isSelected . ' value="' . $option['value'] . '">' . $option['label'] . '</option>';
			}
		} else {
			foreach ( $options as $option )
		...
		case 'color':
			$meta = $meta ? $meta : '#';
			echo '<input type="text" name="' . esc_attr( $name ) . '" class="my-color-field" id="' . esc_attr( $id ) . '" value="' . $meta . '" size="10" />
				<br />' . $desc;
			echo '<script type="text/javascript">
				jQuery(function($) {
					// http://make.wordpress.org/core/2012/11/30/new-color-picker-in-wp-3-5/
				    $("#' . esc_attr( $id ) . '").wpColorPicker();
				});
				</script>';
		break;
...
echo '<select data-placeholder="Select One" name="' . esc_attr( $name ) . '[]" id="' . esc_attr( $id ) . '"' , $type == 'post_chosen' ? ' class="chosen"' : '' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , '>
			...
			foreach ( $posts as $item )
				...
				echo '</select> &nbsp';
			// check if array and print manage links
			if(is_array($post_type)) {
				$count = count($post_type);
				$index = 0;
				foreach ($post_type as $item) {
					 $index++;
					 $post_type_object = get_post_type_object( $item);
					 echo '<span class="description"><a href="' . admin_url( 'edit.php?post_type=' . $item . '">Manage ' . $post_type_object->label ) . '</a></span>';
					 if($count != $index) {
					 	echo ' , ';
					 }
				};
				...
				echo '<span class="description"><a href="' . admin_url( 'edit.php?post_type=' . $post_type . '">Manage ' . $post_type_object->label ) . '</a></span>';
			}
			// echo description
			echo '<br />' . $desc;
			break;
		...
		case 'post_drop_sort':
			...
			echo '</ul>';
		break;
		// tax_select , tax_chosen
		case 'tax_select':
		case 'tax_chosen':
				echo '<select name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"', $type == 'tax_chosen' ? ' class="chosen"' : '' ,'>
						<option value=""></option>'; // Select One
...
class Custom_Add_Meta_Box {
	...
	function save_box( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_chosen', 'tax_checkboxes' ) ) ) {
				// save taxonomies
				if ( isset( $_POST[$field['id']] ) ) {
					$term = $_POST[$field['id']];
					wp_set_object_terms( $post_id, $term, $field['id'] );
				}
			} else if( in_array( $field['type'], array( 'chosen' ) ) ) {

			
			update_post_meta($post_id, $field['id'], array_map( 'strip_tags', $_POST[$field['id']] ) );
